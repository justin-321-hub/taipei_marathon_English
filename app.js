 ...
 * 7) ★ 新增：過濾掉 "Complete" 或 "Complete translation." 的回應
 * 8) ★ 新增：自動將網址轉換為可點擊的連結
 *
 * 依賴：
 * - 頁面需有以下元素：
...
/**
 * ★ 新增：將文字中的網址轉換為可點擊的連結
 * 支援 http://, https://, ftp://, www. 開頭的網址
 */
function linkifyText(text) {
  // 先進行 HTML 實體編碼，防止 XSS 攻擊
  ...
  // 替換網址為連結
  const linkedText = escapedText.replace(urlRegex, function(match) {
    let href = match;
    // 如果是 www. 開頭但沒有協議，加上 http://
    if (match.toLowerCase().startsWith('www.')) {
      href = 'http://' + match;
    }
    // 在新視窗開啟連結，並加上 rel="noopener noreferrer" 增加安全性
    return `<a href="${href}" target="_blank" rel="noopener noreferrer" style="color: #0066cc; text-decoration: underline;">${match}</a>`;
  });
...
function render() {
  ...
  for (const m of messages) {
    ...
    // ★ 改用 innerHTML 並套用 linkifyText 函數
    // 這樣可以正確渲染 HTML 連結
...
/**
 * ★ 新增：檢查文字是否為應該過濾的 Complete 訊息
 */
function isCompleteMessage(text) {
  if (!text || typeof text !== 'string') return false;
  const trimmed = text.trim().toLowerCase();
  return trimmed === 'complete' || 
         trimmed === 'complete.' ||
         trimmed === 'complete translation' ||
         trimmed === 'complete translation.';
}
...
async function sendText(text) {
  ...
  try {
     ...
     * 5) 若回覆是 "Complete" 或 "Complete translation." → 顯示「Please rephrase, thank you.」
     * 6) 其他物件 → JSON 字串化後顯示（利於除錯）
     */
    let replyText;
    ...
    if (isClientIdError) {
      ...
      const textContent = String(data.text || data.message);
      
      // ★ 新增：檢查是否為 Complete 訊息
      if (isCompleteMessage(textContent)) {
        replyText = "Please rephrase, thank you.";
      } else if (data.text === '""' && data.clientId) {
